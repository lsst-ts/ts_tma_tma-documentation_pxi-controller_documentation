...
The cRIO-9145 sends a synchronous signal to the EIB by the NI-9401 module (Mod1/DIO0 and Mod1/DIO1). This signal is synced
with the EtherCAT frequency using the [Input Virtual Point](https://www.ni.com/docs/en-US/bundle/ni-industrial-communications-ethercat/page/ni-9145-fpga-i-o.html),
is automatically generated by the cRIO and is used to trigger a state machine to send the signal to the EIB.

See more details about the `Input Virtual Point` in [ni-9145-timing-diagram](https://www.ni.com/docs/en-US/bundle/ni-industrial-communications-ethercat/page/ni-9145-timing-diagram.html).

If the FPGA code in the cRIO-9145 is running correctly, you should be able to see:

* The cRIO "FPGA LED" flashsing.
* In the NI distribution system (you need to add the ATS AXES PXI to it), the **CyclesCounter** (how many times that the
* EIB has been triggered) increases overtime. This counts the FPGA executions, so as long as it’s increasing everything should be OK.
  The flashing of **FPGACodeRunningClock** in the NI distribution system. This is a true/false clock that will be constantly switching if the FPGA is running.
* If you check the NI distribution system, you can see the **InputVirtualPointTiming** to be 1000. This allows people to
  check the "Input Virtual Point" sampling time, which is in fact an indirect way of measuring the "Input Virtual Point"
  and see that it is working fine. For example a value of 1000 corresponds with 1ms, the sampling rate of Ethercat.

There are indications of the pulse duration from the cRIO to the EIB as well, this are the variables with sample values:
